...
\title{{\bfseries\rmfamily\Huge{高等计算流体力学\hspace{1em}\\第10次作业}}}
...
\section{ENO3有限体积方法测试}

ENO3\cite{shu2020essentially}首先确定$i$单元选用的模板位置。
方便起见在均匀网格上，已知均值$\overline{u}_j$
设原函数为$V_{i+1/2} = \inc x \sum_{j=0}^i{\overline{u}_j}$，则根据文献，
首先直接知道一阶均差为
$$
V[i-1/2,1]=\overline{u}_i
$$
那么左侧方向二阶均差为
$$
V[i-3/2,2]=\frac{1}{2\inc x}(\overline{u}_i-\overline{u}_{i-1})
$$
右侧方向二阶均差为
$$
V[i-1/2,2]=\frac{1}{2\inc x}(\overline{u}_{i+1}-\overline{u}_{i})
$$
进一步，则中央三单元模板的三阶均差为
$$
V[i-3/2,3]=\frac{1}{6\inc x^2}(\overline{u}_{i+1}+\overline{u}_{i-1}-2\overline{u}_i)
$$
则对$i$进行平移可得$\{i-2,i-1,i\}$和$\{i,i+1,i+2\}$的三阶均差。
依据上述公式可先计算二阶均差，根据其绝对大小，选用较小的一侧
决定采用$\{i-1,i\},\{i,i+1\}$之一，
再在二单元模板两侧根据三阶均差的绝对大小选取模板。
确定三单元模板的位置后，即根据有限体积方法插值得到界面值$u_{i-1/2,R},u_{i+1/2,L}$。

由于求解的是标量线性对流，则容易给出迎风通量：
$$
...
计算域为$[-0.5,0.5]$，100单元，采用初值为$|x|<0.25$时$u=1$其余为0的
方波，$CFL=0.5$不同时刻结果如下。

\begin{figure}[H]
    \begin{minipage}[c]{0.45\linewidth}  %需调整
        \centering
        \includegraphics[width=8cm]{T0D1.png}  %需调整
        \caption{$t=0.1$ ENO3 线性对流结果}
        \label{fig:a}
    \end{minipage}
    \hfill %弹性长度
    \begin{minipage}[c]{0.45\linewidth}  %需调整
        \centering
        \includegraphics[width=8cm]{T1D0.png}  %需调整
        \caption{$t=1$ ENO3 线性对流结果}
        \label{fig:b}
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
        ...
        \caption{$t=10$ ENO3 线性对流结果}
        \label{fig:c}
\end{figure}

计算结果可见，ENO方法虽然采用二次多项式插值，有三阶线性精度，
但是其在间断附近依然保持无振荡。间断附近，ENO3将尖锐的波形抹平，
并且随着计算时间增加，间断抹平更加严重，可见ENO方法在间断附近
有特定的非线性数值耗散。
同时，间断附近，ENO格式基本保证单调性，这与文献中TVB性质或许有一定关系。
